...
+ (\dot{m_i} {r_i}^2 + 2 m_i \dot{r_i} ) \dot{\theta_i} + (\dot{m_i} r_i + m_i \dot{r_i}) R \dot{\theta} \cos{(\theta - \theta_i)} - m_i r_i R \dot{\theta} (\dot{\theta} - \dot{\theta_i}) \sin{(\theta - \theta_i)} \\
...
\Lambda(t, \Theta, \dot{\Theta}) = \left( \begin{array}{c}
  ...
  (\dot{m_1} {r_1}^2 + 2 m_1 \dot{r_1} ) \dot{\theta_1} + (\dot{m_1} r_1 + m_1 \dot{r_1}) R \dot{\theta} \cos{(\theta - \theta_1)} - m_1 r_1 ( R \dot{\theta}^2 \sin{(\theta - \theta_1)} + g \cos{\theta_1} ) \\
(\dot{m_2} {r_2}^2 + 2 m_2 \dot{r_2} ) \dot{\theta_2} + (\dot{m_2} r_2 + m_2 \dot{r_2}) R \dot{\theta} \cos{(\theta - \theta_2)} - m_2 r_2 ( R \dot{\theta}^2 \sin{(\theta - \theta_2)} + g \cos{\theta_2} ) \\
  \ddots \\
  (\dot{m_n} {r_n}^2 + 2 m_n \dot{r_n} ) \dot{\theta_n} + (\dot{m_n} r_n + m_n \dot{r_n}) R \dot{\theta} \cos{(\theta - \theta_n)} - m_n r_n ( R \dot{\theta}^2 \sin{(\theta - \theta_n)} + g \cos{\theta_n} )
\end{array} \right) \\
